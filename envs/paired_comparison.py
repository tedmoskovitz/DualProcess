"""based on code provided by Marcel Binz"""
import math
import torch
...
class PairedComparisonTask:

    def __init__(self, num_inputs=4, num_targets=1, direction=False, ranking=False, dichotomized=False, batch_size=1000, num_supports=10, device=None):
        self._task = PairedComparison(num_inputs, num_targets, direction, ranking, dichotomized)
    ...
    def reset(self):
        inputs, targets, _, _ = self._task.get_batch(self.batch_size, self.num_supports, self.device)
        self.step = 0
    ...
    def step(self, action):
        return torch.zeros_like(self._inputs), (action == targets).float().mean().item(), True, {}
